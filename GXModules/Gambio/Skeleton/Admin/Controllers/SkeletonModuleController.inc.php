...
class SkeletonModuleController extends AdminHttpViewController
{
    ...
    {
        $languageTextManager = LanguageTextManager('skeleton_module', $_SESSION['languages_id']);
        ...
        $contentNavigation->add(new StringType($languageTextManager->get_text('HOME')), new StringType('admin.php?do=SkeletonModule'), new BoolType(true));
        $contentNavigation->add(new StringType($languageTextManager->get_text('CONFIGURATION')), new StringType('admin.php?do=SkeletonModule/configuration'), new BoolType(false));
